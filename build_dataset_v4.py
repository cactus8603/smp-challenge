...
- Integrate Qwen2-VL-2B MLLM features: 5 structured category features per image.
  (content_category, dominant_color_tone, scene_complexity, estimated_quality, emotion_trigger)
- Add --mllm_features_dir argument pointing to chunk_*_of_*.jsonl files.
- Fuzzy-map out-of-vocabulary MLLM answers to nearest valid label via difflib.
- Add has_mllm binary flag (1 = image has MLLM features, 0 = missing).

Key changes in v3 (over v2):
- Fix timezone_offset parsing.
- Add has_timezone binary flag.
- Impute missing timezone from lat/lon via TimezoneFinder.
- Add user_description_sentiment (VADER).
...
# ── MLLM feature constants ────────────────────────────────────────────────────
MLLM_COLS = [
    "content_category", "dominant_color_tone", "scene_complexity",
    "estimated_quality", "emotion_trigger",
]
MLLM_VALID_VALUES: Dict[str, Set[str]] = {
    "content_category": {
    ...
    "dominant_color_tone": {"warm", "cool", "neutral", "vibrant", "monochrome"},
    "scene_complexity":    {"simple", "moderate", "complex"},
    "estimated_quality":   {"professional", "amateur", "snapshot"},
    "emotion_trigger":     {
        "joy", "surprise", "curiosity", "inspiration", "humor",
        "awful", "disgust", "angry", "sad", "neutral",
    },
}
...
# -----------------------------------------------------------------------------
# MLLM feature loading
# -----------------------------------------------------------------------------
def _fuzzy_map(value: str, valid_set: Set[str], cutoff: float = 0.6) -> str:
...
def load_mllm_features(mllm_dir: Path) -> pd.DataFrame:
    """Read chunk_*_of_*.jsonl, apply fuzzy mapping, return df indexed by photo_id."""
    rows = []
    for chunk_file in sorted(mllm_dir.glob("chunk_*_of_*.jsonl")):
        with open(chunk_file) as f:
            for line in f:
                ...
                try:
                    ...
                    pass
    if not rows:
        LOGGER.warning("load_mllm_features: no rows found in %s", mllm_dir)
        return pd.DataFrame()
    df = pd.DataFrame(rows)
    ...
    for col, valid_set in MLLM_VALID_VALUES.items():
        if col in df.columns:
            df[col] = df[col].astype(str).map(lambda v, vs=valid_set: _fuzzy_map(v, vs))
    df["photo_id"] = df["photo_id"].astype(str)
    LOGGER.info("load_mllm_features: %d rows loaded from %s", len(df), mllm_dir)
    return df.set_index("photo_id")


def add_mllm_features(df: pd.DataFrame, mllm_df: pd.DataFrame) -> pd.DataFrame:
    """Merge MLLM features into main df on Pid."""
    mllm_feature_cols = [c for c in MLLM_COLS if c in mllm_df.columns]
    if mllm_df.empty or not mllm_feature_cols:
        for col in MLLM_COLS:
            df[f"mllm_{col}"] = None
        df["has_mllm"] = 0
        return df
    pid = df["Pid"].astype(str)
    for col in mllm_feature_cols:
        df[f"mllm_{col}"] = pid.map(mllm_df[col])
    df["has_mllm"] = pid.isin(mllm_df.index).astype(int)
    n_matched = int(df["has_mllm"].sum())
    LOGGER.info("add_mllm_features: %d/%d rows matched (%.1f%%)",
...
# -----------------------------------------------------------------------------
def load_split(input_dir: Path, split: str,
               mllm_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    prefix = split
    ...
    out = add_reverse_geo_features(out)   # lat/lon → city/state/country (has_geo=1 only)
    ...
    # v4: MLLM features
    if mllm_df is not None and not mllm_df.empty:
        out = add_mllm_features(out, mllm_df)
    else:
        for col in MLLM_COLS:
            out[f"mllm_{col}"] = None
        out["has_mllm"] = 0
    ...
    front = [
        ...
        # v4: MLLM features
        "has_mllm",
        "mllm_content_category", "mllm_dominant_color_tone", "mllm_scene_complexity",
        "mllm_estimated_quality", "mllm_emotion_trigger",
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build unified SMP dataset tables, v4: MLLM features + reverse geocoding.")
    ...
    parser.add_argument(
        "--mllm_features_dir", type=str, default=None,
        help="Directory containing chunk_*_of_*.jsonl MLLM feature files (optional).",
    )
...
def main() -> None:
    ...
    mllm_df: Optional[pd.DataFrame] = None
    if args.mllm_features_dir is not None:
        mllm_dir = Path(args.mllm_features_dir)
        LOGGER.info("Loading MLLM features from %s", mllm_dir)
        mllm_df = load_mllm_features(mllm_dir)
    else:
        LOGGER.info("--mllm_features_dir not set; MLLM columns will be None")

    official_train_df = load_split(input_dir, "train", mllm_df=mllm_df)
    test_df = load_split(input_dir, "test", mllm_df=mllm_df)
...
# Example:
# python3 build_dataset_v3.py --input_dir /local/smp/data --output_dir /local/smp/processed_v3 --split_mode full_train

...
- output a full official_train table for later GroupKFold / ensemble training
- optionally still support a legacy single train/val split mode
- IMPORTANT:
    * in full_train mode, DO NOT precompute label-derived user aggregate features
      on the whole official train, otherwise KFold validation will leak labels
...
def add_user_aggregate_features(train_df: pd.DataFrame, target_df: pd.DataFrame) -> pd.DataFrame:
    """
    Fit aggregate stats on train_df and apply to target_df.
    Keep aligned output columns across train / val / test / official_train.
    """
...
def load_split(input_dir: Path, split: str) -> pd.DataFrame:
    ...
    front = [
        ...
        "user_description", "location_description",
        # NOTE:
        # The label-derived user aggregate columns below must NOT be included
        # in full_train mode before KFold split, otherwise validation leakage happens.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--val_ratio", type=float, default=0.1, help="Validation ratio for single_split mode.")
    parser.add_argument("--split_seed", type=int, default=42, help="Random seed for single_split mode.")
    parser.add_argument("--split_by", type=str, default="user", choices=["user", "post"], help="single_split mode only.")
...
def main() -> None:
    ...
    # IMPORTANT:
    # In full_train mode, DO NOT compute label-derived user aggregate features here.
    # Otherwise those columns leak validation labels after KFold split.
    if args.split_mode == "full_train":
        ...
        LOGGER.info("Done.")
        LOGGER.info("Official train shape: %s", official_train_df.shape)
        LOGGER.info("Test shape: %s", test_df.shape)
        LOGGER.info("Next step: in train.py, after fold split, recompute user aggregate features using fold-train only.")
        return

    # legacy / convenience mode
    # In single_split mode, computing user aggregates from train split is OK.
...
# Example:
# 1) For future GroupKFold training:
# python3 build_dataset_v1.py --input_dir /local/smp/data --output_dir ./data/processed --split_mode full_train
#
# 2) For quick single validation:
# python3 build_dataset_v1.py --input_dir /local/smp/data --output_dir ./data/processed --split_mode single_split --val_ratio 0.1 --split_by user
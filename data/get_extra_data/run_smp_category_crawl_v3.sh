#!/usr/bin/env bash
# run_smp_category_crawl_v3.sh
#
# V3 changes vs V2:
#   - Parallel execution: each category runs in background simultaneously
#   - Each category gets its own log file under $LOG_DIR
#   - Travel&Active&Sports is skipped (already done)
#   - Slightly longer sleep to be safe with parallel API calls
#
# Usage:
#   ./run_smp_category_crawl_v3.sh
#
...
#   tail -f /local/smp/logs/crawl_Food.log
#
# Check all logs at once:
#   tail -n 5 /local/smp/logs/crawl_*.log
#
# Dry run:
#   TOTAL_ITEMS=5000 DOWNLOAD_IMAGES=0 TIME_SLICE=0 ./run_smp_category_crawl_v3.sh
...
SLEEP_MIN="${SLEEP_MIN:-1.5}"      # 延長 sleep，平行跑時避免打爆 API
...
# ──────────────────────────────────────────────────────
# Categories — Travel skipped (already done at 147%)
# ──────────────────────────────────────────────────────
CATEGORIES=(
...
# ──────────────────────────────────────────────────────
# Per-category crawl function (runs in background)
# ──────────────────────────────────────────────────────
crawl_category() {
    local entry="$1"
...

    {
        ...
        for q in "${query_list[@]}"; do
            ...
            if [ "$existing" -ge "$current_target" ]; then
                echo "[$idx/$num_queries] Skip: $q (existing=$existing >= target=$current_target)"
                continue
            fi
            ...
            echo "[$idx/$num_queries] Done: $q | items now: $(count_existing_items "$output_dir")"
        done
...
export -f crawl_category sanitize_name scale_target download_flag time_slice_flag fetch_detail_flag count_existing_items
...
cat <<INFO
========================================
SMP Flickr category crawl V3 (parallel)
========================================
...
for entry in "${CATEGORIES[@]}"; do
    ...
    echo "Launching: $category  →  logs/${safe}.log"
    crawl_category "$entry" &
    PIDS+=($!)
done

echo ""
echo "All ${#CATEGORIES[@]} categories launched."
...
for i in "${!PIDS[@]}"; do
    ...
    if wait "$pid"; then
        ...
        echo "❌  Failed: $category (exit code $?)"
        FAILED+=("$category")
    fi
done

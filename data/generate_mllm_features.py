#!/usr/bin/env python3
"""
Qwen2-VL-2B 全量推論：對 305k 張訓練圖 + test 圖生成 5 個 MLLM category 特徵
多 GPU 並行用法：
    CUDA_VISIBLE_DEVICES=3 python3 data/generate_mllm_features.py --chunk 0 --n_chunks 3 &
    CUDA_VISIBLE_DEVICES=4 python3 data/generate_mllm_features.py --chunk 1 --n_chunks 3 &
    CUDA_VISIBLE_DEVICES=5 python3 data/generate_mllm_features.py --chunk 2 --n_chunks 3 &
"""
...
TRAIN_ROOT   = Path("/ssd1/lchiayu/smp-challenge/data/official_data/train_set/train")
TEST_ROOT    = Path("/ssd1/lchiayu/smp-challenge/data/official_data/test_set/test")
TRAIN_TXT    = Path("/ssd1/lchiayu/smp-challenge/data/official_data/smp_input/train/train_img_filepath.txt")
TEST_TXT     = Path("/ssd1/lchiayu/smp-challenge/data/official_data/smp_input/test/test_img_filepath.txt")
OUTPUT_DIR   = Path("/ssd1/lchiayu/smp-challenge/data/mllm_features")
...
def load_paths(txt_path: Path, img_root: Path) -> list[tuple[str, Path]]:
    """讀取 filepath txt，回傳 (photo_id, abs_path) 列表，跳過不存在的檔案"""
    entries = []
    with open(txt_path) as f:
        for line in f:
            rel = line.strip()
            parts = rel.split("/")
            photo_id = Path(parts[-1]).stem   # e.g. "775"
            uid      = parts[-2]              # e.g. "59@N75"
            abs_path = img_root / uid / f"{photo_id}.jpg"
            if abs_path.exists():
                entries.append((photo_id, abs_path))
    return entries
...
def main():
    ...
    if args.split in ("train", "all"):
        entries += [(pid, p, "train") for pid, p in load_paths(TRAIN_TXT, TRAIN_ROOT)]
    if args.split in ("test", "all") and TEST_TXT.exists():
        entries += [(pid, p, "test")  for pid, p in load_paths(TEST_TXT,  TEST_ROOT)]

    # 分 chunk
    chunk_entries = [e for i, e in enumerate(entries) if i % args.n_chunks == args.chunk]
    todo = [(pid, p, sp) for pid, p, sp in chunk_entries if pid not in done_ids]
    ...
    with open(output_path, "a") as fout:
        for i, (photo_id, img_path, split) in enumerate(todo):
            try:
                row = {"photo_id": photo_id, "split": split}
                ...
                errors += 1
                fout.write(json.dumps({"photo_id": photo_id, "split": split, "error": str(e)}) + "\n")
                fout.flush()
